<?php $this->load->view('layouts/beforeContent', ['active_page' => 'keanggotaan']); ?>

<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
	<div class="d-flex justify-content-between">
		<div>
			<h3>Daftar Anggota</h3>
			<p>Berikut adalah daftar daftar anggota koperasi:</p>
		</div>
		<div>
	...
	<div class="card my-3">
		<div class="card-body">
			<table class="table table-hover">
				...
				<tbody>
					<?php for ($i = 0; $i < count($anggotas); $i++) : $r = $anggotas[$i];
			...
			<i>*Pinjaman harus dibayar lunas sebelum anggota dapat dihapus</i>
		</div>
	</div>
</main>
<!-- Modal Section Begin -->
	...
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="modal">Tambah Anggota Baru</h5>
...
<script>
	...
	$('form').submit(function(e) {
		...
		$.ajax({
			url: '<?= base_url('api/user/add/anggota') ?>',
			type: "POST",
	...
	function hapus(user_id) {
		$.ajax({
			url: '<?= base_url('api/user/delete/anggota') ?>',
			type: 'POST',
			data: {
				user_id,
				submit: true
